...
- `flake.nix` 定义 `nixosConfigurations.x`，`flake.lock` 锁定输入版本。
...
- `sudo nixos-rebuild dry-activate --flake .#x`：生成并验证配置，不实际切换。
- `sudo nixos-rebuild test --flake .#x`：临时切换用于验证；确认无误后再用 `switch`。
- `sudo nixos-rebuild switch --flake .#x`：正式切换到新配置。
